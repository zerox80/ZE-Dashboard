    ...
    return (
        <div className="app-page">
            ...
            <section className="surface overflow-hidden">
                ...
                {filtered.length ? <>
                    <div className="hidden grid-cols-[minmax(240px,1.5fr)_140px_140px_minmax(120px,.7fr)_108px] gap-4 border-y border-white/[0.06] bg-white/[0.02] px-6 py-2.5 text-[10px] font-bold uppercase tracking-[.14em] text-[#5e6878] xl:grid"><span>Rechnung</span><span>Datum</span><span>Status</span><span className="text-right">Betrag</span><span /></div>
                    {filtered.map((invoice) => <article key={invoice.id} className="data-row grid-cols-[minmax(0,1fr)_auto] xl:grid-cols-[minmax(240px,1.5fr)_140px_140px_minmax(120px,.7fr)_108px] xl:gap-4">
                        ...
                        <div className="col-span-2 grid grid-cols-1 gap-2 border-t border-white/[0.06] pt-3 sm:grid-cols-3 xl:col-span-1 xl:contents xl:border-0 xl:pt-0">
                            <p className="flex items-center justify-between gap-2 text-sm muted xl:block"><span className="text-[10px] font-bold uppercase tracking-wider text-[#5e6878] xl:hidden">Datum</span>{invoice.start_date ? new Date(invoice.start_date).toLocaleDateString('de-DE') : '–'}</p>
                            <div className="flex items-center justify-between gap-2 xl:block"><span className="text-[10px] font-bold uppercase tracking-wider text-[#5e6878] xl:hidden">Status</span><span className={`chip w-fit ${invoice.is_protected ? 'border-[#b28cff]/15 bg-[#b28cff]/10 text-[#c9adff]' : 'border-[#b8f15a]/15 bg-[#b8f15a]/10 text-[#b8f15a]'}`}>{invoice.is_protected ? 'Geschützt' : 'Erfasst'}</span></div>
                            <div className="flex items-center justify-between gap-2 xl:block"><span className="text-[10px] font-bold uppercase tracking-wider text-[#5e6878] xl:hidden">Betrag</span><p className="text-base font-bold text-white xl:text-right">{invoice.value != null ? `${formatGermanNumber(invoice.value)} €` : '–'}</p></div>
                        </div>
                        <div className="col-start-2 row-start-1 flex shrink-0 justify-end gap-1 self-center xl:col-auto xl:row-auto"><button onClick={() => handleDownload(invoice)} className="icon-btn" title="Herunterladen"><FiDownload /></button>{invoice.can_write && <button onClick={() => { setEditingInvoice(invoice); setIsUploadOpen(true) }} className="icon-btn" title="Bearbeiten"><FiEdit3 /></button>}{invoice.can_delete && <button onClick={() => handleDelete(invoice)} disabled={invoice.is_protected} className="icon-btn hover:text-red-300 disabled:cursor-not-allowed disabled:opacity-30" title="Löschen"><FiTrash2 /></button>}</div>
                    </article>)}
